...
私有的单用户短链接服务。Go 提供公开跳转、登录和管理 API；Vue 管理界面位于 `/admin/`，根路径 `/` 的行为可在设置中配置，默认跳转至 `/admin/`。创建、删除、设置等管理操作需要登录，访问有效短码时无需登录。
...
- 根路径可跳转至 `/admin/`、返回 404、跳转至其他 HTTP/HTTPS 网址，或返回自定义 HTML。目标填写域名时自动补全 `https://`。自定义 HTML 最多 256 KB，并以隔离来源运行。
